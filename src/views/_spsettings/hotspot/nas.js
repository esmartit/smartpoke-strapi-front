...
import nasService from "../../../services/settings/nas.service";

import nasTypes from "../../../data/_settings/nasType";
...
function NasTable() {
  ...
  const [alertMsg, setAlertMsg] = useState({
    ...
    text: "text-danger",
    icon: "fas fa-ban",
    msg: "Am error occured while trying to request NAS.",
  });
  ...
  const getErrorMessage = (err) => {
    ...
    switch (err) {
      ...
      default:
        alertMsg.msg =
          "An error occured while trying to request NAS.";
        break;
    }
    ...
    return {
      ...
      actions: (
        ...
        <div className="text-center">
          ...
          <Button
            onClick={() => {
              let item = Nas.find((o) => o.id === nas.id);
              setModal(!modal);
              setData(item);
            }}
            color="inverse"
      ...
      <Modal isOpen={modalDelete} toggle={toggleDelete.bind(null)}>
        ...
        <ModalBody>
          <Form onSubmit={(event) => handleSubmitDelete(event)}>
            <Input type="hidden" name="id" id="id" defaultValue={data.id} />
            <div>
              Do you want to delete? <b>{data.nasname}</b>
            </div>
            <br />
            <FormGroup>
              <Button color="success" type="submit">
