...
import cityService from "../../../_spservices/_spmaintenances/_splocations/cities.service";
...
function CityTable() {
  ...
  const getErrorMessage = (err) => {
    ...
    switch (err) {
      ...
      default:
        alertMsg.msg =
          "An error occured while trying to request cities.";
        break;
    }
  ...
  const Cities = cityList.map((city) => {
    let countryId = city.attributes.state.data.attributes.country.data ? city.attributes.state.data.attributes.country.data.id : "";
    let countryName = city.attributes.state.data.attributes.country.data ? city.attributes.state.data.attributes.country.data.attributes.name : "";
    let stateId = city.attributes.state.data ? city.attributes.state.data.id : "";
    let stateName = city.attributes.state.data ? city.attributes.state.data.attributes.name : "";
    ...
    return {
      ...
      stateId: stateId,
      stateName: stateName,
      cityCode: city.attributes.cityCode,
      name: city.attributes.name,
      zipCodes: city.attributes.zip_codes.data,
      spots: city.attributes.spots.data,
      actions: (
        ...
        <div className="text-center">
          ...
          <Button
            onClick={() => {
              let item = Cities.find((o) => o.id === city.id);
              setModal(!modal);
        ...
        <ModalBody>
          <Form onSubmit={(event) => handleSubmitDelete(event)}>
            ...
            {!noSpot || !noZipCode ? (
              ...
              <div>
                You can not delete the city <b>{data.name}</b>, it has zip codes and/or
                spots associated.
              </div>
            )}
            <br />
